...
//            $table->searchable(['first_name']);
            ...
            $table->column('approve', function ($column)
            {
                ...
                $column->value = function ($row) {
                    ...
                    $html = [
                        app('html')->raw('<a href="/admin/wingz/instructors/aircraft/'.$row->id.'/edit" style="color: '.$color.'"><i class="fa fa-check-circle-o"></i></a>')
                    ];
    ...
    public function actions(TableBuilder $table)
    {
        return $table->extend(function (TableGrid $table) {
            $table->column('action')
                ...
                ->value(function ($row) {
                    $html = [
                        app('html')->raw('<a href="/admin/wingz/instructors/instructor/'.$row->id.'/edit"><i class="fa  fa-edit"></i></a>')
                    ];
    ...
    public function frontendForm($model)
    {
        ...
        $form = $this->form->of('wingz.instructor', function (FormGrid $form) use ($model)  {
            ...
            $form->fieldset('Instructor\'s Company', function ($form) {
                $instructors = Instructors::lists('name', 'id');

                $form->control('input:select', 'company')->label('')->attributes(['placeholder'=>'Company'])->options($instructors);
            });
