...
use Wingz\Renters\Model\Renters;
use Wingz\Renters\Model\Renters as Eloquent;
        ...
        return $this->table->of('wingz.aircraft', function (TableGrid $table) use ($model) {
            ...
            $table->column('approve', function ($column)
            {
                ...
                $column->value = function ($row) {
                    ...
                    $html = [
                        app('html')->raw('<a href="/admin/wingz/renters/aircraft/'.$row->id.'/edit" style="color: '.$color.'"><i class="fa fa-check-circle-o"></i></a>')
                    ];
    ...
    public function actions(TableBuilder $table)
    {
        return $table->extend(function (TableGrid $table) {
            $table->column('action')
                ...
                ->value(function ($row) {
                    $html = [
                        app('html')->raw('<a href="/admin/wingz/renters/instructor/'.$row->id.'/edit"><i class="fa  fa-edit"></i></a>')
                    ];
    ...
    public function frontendForm($model)
    {
        ...
        $form = $this->form->of('wingz.instructor', function (FormGrid $form) use ($model)  {
            ...
            $form->fieldset('Instructor\'s Company', function ($form) {
                ...
                $form->control('input:select', 'company')->label('')->attributes(['placeholder'=>'Company'])->options($renters);
            });
