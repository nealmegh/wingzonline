...
use Wingz\Foundation\Model\Airport as Eloquent;
...
class AirportPresenter extends Presenter
{
    /**
    ...
    public function table($model)
    {
        return $this->table->of('wingz.aircraft', function (TableGrid $table) use ($model) {
            ...
            // Add columns.
            $table->column('Airport ID', 'airport_id');
            $table->column('Airport Name', 'airport_name');
        ...
        return $table->extend(function (TableGrid $table) {
            $table->column('action')
                ...
                ->value(function ($row) {
                    $html = [
                        app('html')->link(
                            handles("orchestra::wingz/companies/airport/{$row->id}/edit"),
    ...
    public function form(Eloquent $model)
    {
        ...
        $form = $this->form->of('wingz.airport', function (FormGrid $form) use ($model)  {

            $form->setup($this, 'orchestra::wingz/companies/airport', $model);

            $form->fieldset(function ($form) {

                $form->control('input:text', 'airport_name')->label('Airport Name:');
